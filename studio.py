...
from Beta import change_format
  ...
  for i in range(len(df)):
    ...
    for j in range(1,100):
      ...
      if current_price<(temp+ath_price/100) and current_price>temp:
        a=[str(df.iloc[i]["t"]),"T",temp,df.iloc[i]["partitions"][j],df.iloc[i]["partitions"][j]/total_supply,ap]
      else:
        if ap<=0.25:
          a=[str(df.iloc[i]["t"]),"F1",temp,df.iloc[i]["partitions"][j],df.iloc[i]["partitions"][j]/total_supply,ap]
        elif ap>0.25 and ap<=0.5:
          a=[str(df.iloc[i]["t"]),"F2",temp,df.iloc[i]["partitions"][j],df.iloc[i]["partitions"][j]/total_supply,ap]
        elif ap>0.5 and ap<=0.75:
          a=[str(df.iloc[i]["t"]),"F3",temp,df.iloc[i]["partitions"][j],df.iloc[i]["partitions"][j]/total_supply,ap]
        else:
          a=[str(df.iloc[i]["t"]),"F4",temp,df.iloc[i]["partitions"][j],df.iloc[i]["partitions"][j]/total_supply,ap]
      list.append(a)
  df2 = pd.DataFrame(list, columns =['t',"color",'Price', 'Distribution',"Percentage","AP"])
  fig = px.bar(df2, x="Price", y="Percentage", color="color",animation_frame="t", hover_name="t", hover_data={"color":False,"t":False,"Percentage":":.2%","AP":":.2%"})
  fig.update_layout(
...
def URPD():
  l=[ "URPD (ATH-Partitioned) ", "BTC", "/v1/metrics/indicators/utxo_realized_price_distribution_ath", "24h", "NATIVE",1]
  two,symbol,addresses,intervel,currency,movingaverage=l[0],l[1],l[2],l[3],l[4],l[5]
  with st.expander("Setting"):
...
def HODLtable(df,two):
  df=df.set_index("t")
  with st.expander("Setting"):
    timechoose=st.date_input("Start",list(df.index)[-11],min_value=list(df.index)[0], max_value=list(df.index)[-1],key="timechoose")
    timechoose2=st.date_input("End",list(df.index)[-1],min_value=list(df.index)[0], max_value=list(df.index)[-1],key="timechoose2")
  df=df[glistdic[two]]
