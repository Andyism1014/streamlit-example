...
def main():
    dfp=get_g("BTC","/v1/metrics/market/price_usd_close","24h",10000)
    st.header("Active Entities（活跃个体）")
    st.write(show_cp(get_g("BTC","/v1/metrics/entities/active_count","24h",10000),dfp))
    ...
    st.write(show_cp2(get_g("BTC","/v1/metrics/transactions/transfers_volume_by_size_entity_adjusted_sum","24h",10000),dfp,l1))
    ...
    st.write(show_cp2(get_g("BTC","/v1/metrics/supply/rcap_hodl_waves","24h",10000),dfp,l2))
    st.header("Futures Open Interest Perpetual（永续合约持仓金额）")
    st.write(show_cp(get_g("BTC","/v1/metrics/derivatives/futures_open_interest_perpetual_sum","24h",10000),dfp))
    st.header("Futures Open Interest（交割合约持仓金额）")
    st.write(show_cp(get_g("BTC","/v1/metrics/derivatives/futures_open_interest_sum","24h",10000),dfp))
    st.header("Number of Whales（巨鲸人数）")
    st.write(show_cp(get_g("BTC","/v1/metrics/entities/min_1k_count","24h",10000),dfp))
    ...
    st.write(show_cp2(get_g("BTC","/v1/metrics/entities/supply_distribution_relative","24h",10000),dfp,l3))
    st.header("Futures Perpetual Funding Rate（资金费率）")
    st.write(show_cp(get_g("BTC","/v1/metrics/derivatives/futures_funding_rate_perpetual","24h",10000),dfp))
    st.header("Futures Estimated Leverage Ratio（杠杆率）")
    st.write(show_cp(get_g("BTC","/v1/metrics/derivatives/futures_estimated_leverage_ratio","24h",10000),dfp))
    st.header("Stablecoin Supply Ratio（交易所BTC与稳定币比值）")
    st.write(show_cp(get_g("BTC","/v1/metrics/indicators/ssr","24h",10000)[["t","o_v"]].rename(columns={"o_v":"v"}),dfp))